...
class BootBot extends EventEmitter {
  ...
  constructor(options) 
  {
    ...
    //}
    this.messagequeuer = options.messagequeuer;
  ...
  setHomeUrl(url)
  {
    ...
    return this.sendProfileRequest({ home_url });
  }
  ...
  resetkeywords()
  {
    this._hearMap = []; return this._hearMap.length;
  }
  ...
  _handleMessageEvent(event)
  {
    ...
    this._hearMap.forEach(hear => 
    {
      if (typeof hear.keyword === 'string' && hear.keyword.toLowerCase() === text.toLowerCase()) 
      {
        ...
        this.messagequeuer.insertmessage(event, hear.keyword);
    ...
    data.entry.forEach((entry) => 
    {
      ...
      if (entry.messaging)
      {
          ...
          entry.messaging.forEach((event) => 
          {
            ...
            {
              this.messagequeuer.insertmessage(event, "", "true");
              return;
            }
            ...
            } else if (event.delivery) 
            {
              ...
              this.messagequeuer.insertevent(event);
  ...
  _verifyRequestSignature(req, res, buf) {
    ...
    if (!signature) 
    {
      throw new Error('Couldn\'t validate the request signature. #1');
    } else 
    {
      ...
      console.log("Secret: " + this.appSecret);
      console.log("Signature: " + signatureHash);
      console.log("Result: " + expectedHash);

      if (signatureHash != expectedHash) 
      {
        //throw new Error('Couldn\'t validate the request signature. #2');
      }
    }
  }
}
